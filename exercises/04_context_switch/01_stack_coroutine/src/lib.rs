...
impl TaskContext {
    ...
    pub fn init(&mut self, stack_top: usize, entry: usize) {
        ...
        self.sp = (stack_top & !0xf) as u64;//16字节对齐
        for s in [&mut self.s0, &mut self.s1, &mut self.s2, &mut self.s3, &mut self.s4, &mut self.s5, &mut self.s6, &mut self.s7, &mut self.s8, &mut self.s9, &mut self.s10, &mut self.s11] {
            *s = 0;
        }
    }
}// 先设置ra和sp，s0-s11在switch_context里保存/恢复，其中ra保存了entry地址，sp保存了stack_top地址

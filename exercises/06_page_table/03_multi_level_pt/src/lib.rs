...
impl Sv39PageTable {
    ...
    pub fn extract_vpn(va: u64, level: usize) -> usize {
        ...
        return ((va >> (12 + level * 9)) & 0x1FF) as usize;
    }
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pet2 = self.nodes.get_mut(&self.root_ppn).unwrap().entries[vpn2];
        let level1_ppn = if pet2 & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&self.root_ppn).unwrap().entries[vpn2] =
                (new_ppn << PPN_SHIFT) | PTE_V;
            new_ppn
        } else {
            pet2 >> PPN_SHIFT
        };
        let pet1 = self.nodes.get_mut(&level1_ppn).unwrap().entries[vpn1];
        let level0_ppn = if pet1 & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&level1_ppn).unwrap().entries[vpn1] =
                (new_ppn << PPN_SHIFT) | PTE_V;
            new_ppn
        } else {
            pet1 >> PPN_SHIFT
        };
       self.nodes.get_mut(&level0_ppn).unwrap().entries[vpn0] =
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..=2).rev() {
            ...
            let pet = self.nodes.get(&current_ppn).unwrap().entries[vpn];
            if pet & PTE_V == 0 {
                return TranslateResult::PageFault;
            }
            if pet & (PTE_R | PTE_W | PTE_X) != 0 {
                let offset_bits = 12 + level * 9;
let offset_mask = (1u64 << offset_bits) - 1;
let pa = ((pet >> PPN_SHIFT) << 12) | (va & offset_mask);
                return TranslateResult::Ok(pa);
            } else {
                current_ppn = pet >> PPN_SHIFT;
            }
        };

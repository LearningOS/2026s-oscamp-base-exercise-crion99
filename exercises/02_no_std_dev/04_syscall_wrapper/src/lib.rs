...
pub unsafe fn syscall3(id: usize, arg0: usize, arg1: usize, arg2: usize) -> isize {
    ...
    core::arch::asm!(
        "syscall",
        inlateout("rax") id as isize => ret,//把id的值传入rax寄存器，并把rax寄存器的值传出到ret变量
        in("rdi") arg0,
        in("rsi") arg1,
        in("rdx") arg2,//吧rdx的值传入agr2寄存器
        out("rcx") _,
        out("r11") _,//释放rcx和r11寄存器
    );
    ret
    //   - "syscall" instruction
...
pub fn sys_read(fd: usize, buf: &mut [u8]) -> isize {
    ...
    unsafe { syscall3(0,fd,buf.as_mut_ptr() as usize, buf.len()) }
}

...
impl<T> RwLock<T> {
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            ...
            if self.state.compare_exchange(WRITER_WAITING, WRITER_HOLDING, Ordering::AcqRel, Ordering::Acquire).is_ok()
            {
                break;
            }
            if self.state.compare_exchange(0, WRITER_HOLDING, Ordering::AcqRel, Ordering::Acquire).is_ok()
            {
                break;
            }
        }
